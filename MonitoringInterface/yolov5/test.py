...
@torch.no_grad()
def run(weights=ROOT / 'runs/train/exp6/weight/bast.pt',  # model.pt path(s)
    ...
    if webcam:
        ...
        dataset = LoadImages(source, img_size=imgsz, stride=stride, auto=pt)  # 从文件夹中获取img或video
        bs = 1  # batch_size
...
def LoadModel(weights=ROOT / './runs/train/exp6/weights/best.pt',  # model.pt path(s)
              data=ROOT / 'data/coco128.yaml',  # dataset.yaml path
              device='',  # cuda device, i.e. 0 or 0,1,2,3 or cpu
              half=False,  # use FP16 half-precision inference 半精度浮点数
              dnn=False,  # use OpenCV DNN for ONNX inference
